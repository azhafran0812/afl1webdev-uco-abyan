...
@section('content')
    <div class="space-y-6">
        ...
        <div
            class="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white/50 backdrop-blur-sm p-4 rounded-lg">
            <div>
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('categories.create') }}"
                    class="inline-flex items-center px-4 py-3 bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 font-bold rounded-lg shadow-sm transition">
                ...
                <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                    ...
                    <div class="md:col-span-3">
                        ...
                        <select name="category_id"
                            class="w-full py-2 px-3 border border-gray-300 rounded-lg focus:ring-black focus:border-black text-sm bg-white cursor-pointer">
                    ...
                    <div class="md:col-span-2 flex gap-2">
                        <select name="sort"
                            class="w-full py-2 px-2 border border-gray-300 rounded-lg focus:ring-black focus:border-black text-sm bg-white cursor-pointer"
                            title="Urutkan">
                        ...
                        <button type="submit"
                            class="bg-gray-900 hover:bg-black text-white font-bold py-2 px-4 rounded-lg shadow transition">
                            Go
                        </button>
                    </div>
                </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            @forelse($products as $item)
                <div
                    ...
                    <div class="h-48 bg-gray-100 overflow-hidden relative">
                        ...
                        @if ($item->category)
                            <span
                                class="absolute top-2 right-2 bg-white/90 backdrop-blur text-gray-800 text-[10px] font-bold px-2 py-1 rounded shadow-sm border border-gray-200 uppercase tracking-wider">
                                {{ $item->category->name }}
                            </span>
                        @endif
                    </div>
                    ...
                    <div class="p-5 flex-1 flex flex-col">
                        <div class="mb-2">
                            @if ($item->category)
                                <span
                                    class="inline-block px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide border text-gray-600 bg-gray-100 border-gray-200">
                                    {{ $item->category->name }}
                                </span>
                            @else
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                @auth
                                    <form action="{{ route('cart.add', $item->id) }}" method="POST">
                                        @csrf
                                        <button type="submit"
                                            class="p-2 bg-black text-white rounded-lg hover:bg-gray-800 transition shadow-sm"
            ...
            @empty
                <div
                    class="col-span-full flex flex-col items-center justify-center py-16 text-center bg-gray-50 border-2 border-dashed border-gray-300 rounded-xl">
                    ...
                    <a href="{{ route('products.create') }}"
                        class="mt-4 inline-flex items-center px-4 py-2 bg-black text-white rounded hover:bg-gray-800 transition">
                        + Buat Produk
                    </a>
                </div>
            @endforelse
